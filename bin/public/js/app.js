...
const app = new Vue({
  ...
  methods: {
    ...
    riscoMews(item) {
      return((item.STATUS == 2) && (parseInt(this.getScore('MEWS', item)) >= 4));
    },
    titulo(item) {
      if(item.STATUS == 1) {
        return 'Vago'
    ...
    calcVagos(leitosDaUnidade) {
      ...
      return(isNaN(leitosVagos) ? 0 : leitosVagos)
    },
    isMobile() {
      var check = false;
        ...
        try{
          Object.keys(item.SCORES).forEach(scoreName => {
            ...
            if((scoreName == "MEWS") && (parseInt(this.getScore('MEWS', item)) >= 4))
              content += `<strong style="color:red;background-color:yellow;font-size:1em">${scoreName}: ${scoreVal} (ALERTA)</strong><br>`;
            else
